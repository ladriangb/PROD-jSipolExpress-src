...
public class HistorialPersonaDetailController extends DefaultDetailFrameController {
    ...
    public Response loadData(Class valueObjectClass) {
        ...
        if (rif != null) {
            ...
            sql = "FROM " + Recibo.class.getName() + " R WHERE ";
            sql += " R.asegurado.rif.rif=:ci ";

            q = s.createQuery(sql);
            q.setString("ci", rif);

            Set<Recibo> recib = new HashSet<Recibo>(q.list());

            sql = "FROM " + Recibo.class.getName() + " R  JOIN R.beneficiarios2 B WHERE ";
            sql += " B.beneficiario.rif.rif=:ci ";
            recib.addAll(s.createQuery(sql).setString("ci", rif).list());


            List<Recibo> recibos = new ArrayList(recib);
            if (recibos.size() != 0) {
                re = true;
